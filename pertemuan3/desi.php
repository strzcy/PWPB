...
<body>
  ...
  <div class="data-display">
    ...
    if (isset($_POST['submit_data'])) {
        ...
        $umur = (int)$_POST['umur'];
        $siswaBaru = ["nama" => $nama, "kelas" => $kelas, "umur" => $umur];
        array_push($_SESSION['daftar_siswa'], $siswaBaru);
        echo "<p class='message' style='color: green;'>Data siswa " . $nama . " berhasil ditambahkan!</p>";
    }

    // Hapus siswa berdasarkan key
    if (isset($_POST['delete_key'])) {
        $key = (int)$_POST['delete_key'];
        if (isset($_SESSION['daftar_siswa'][$key])) {
            unset($_SESSION['daftar_siswa'][$key]);
            $_SESSION['daftar_siswa'] = array_values($_SESSION['daftar_siswa']);
            echo "<p class='message' style='color: red;'>Data siswa berhasil dihapus!</p>";
        }
    }

    if (!empty($_SESSION['daftar_siswa'])) {
        echo "<table><thead><tr><th>No.</th><th>Nama</th><th>Kelas</th><th>Umur</th><th>Aksi</th></tr></thead><tbody>";
        foreach ($_SESSION['daftar_siswa'] as $idx => $data) {
            echo "<tr>";
            echo "<td>" . ($idx + 1) . "</td>";
            ...
            echo "<td>
                    <form method='post' style='margin:0;'>
                      <button type='submit' name='delete_key' value='$idx' class='delete-btn'>Hapus</button>
                    </form>
                  </td>";
            echo "</tr>";
        }
